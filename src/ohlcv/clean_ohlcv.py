...
class CleanOHLCV:
    ...
    def _standardize_datetime_index(self, index: pd.Index) -> pd.DatetimeIndex:
        """Return a timezone-naive, date-only index for consistent alignment."""

        normalized_idx = pd.DatetimeIndex(index)
        if normalized_idx.tz is not None:
            normalized_idx = normalized_idx.tz_convert(None)
        return normalized_idx.normalize()
    ...
    def _get_aligned_index(
        ...
        for df in data_dict.values():
            if df.empty:
                continue
            normalized_indexes.append(self._standardize_datetime_index(df.index))

        if not normalized_indexes:
            raise ValueError("Cannot align empty data frames")

        # Find earliest start and latest end using normalized indexes
        min_date = min(idx.min() for idx in normalized_indexes)
        max_date = max(idx.max() for idx in normalized_indexes)

        # Create continuous index (already normalized)
    ...
    def _clean_single(
        ...
        # Normalize the index so reindexing matches the union range
        df = df.copy()
        df.index = self._standardize_datetime_index(df.index)
