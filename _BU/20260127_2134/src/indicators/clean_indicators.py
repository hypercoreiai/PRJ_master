...
def _to_datetime_index(df: pd.DataFrame, date_col: str | None) -> pd.DataFrame:
    """Ensure df has a DatetimeIndex; use date_col if provided, else index."""
...
class CleanIndicators:
    ...
    def _standardize_one(
        ...
        # Drop any leftover non-datetime columns that might have been index names
    ...
    def _broadcast_point(self, name: str, df: pd.DataFrame, full_index: pd.DatetimeIndex) -> pd.DataFrame:
        ...
        try:
            ...
            v = float("nan")
        out = pd.DataFrame({name: v}, index=full_index)
        out.index.name = "date"
        return out
